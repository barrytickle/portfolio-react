    ...
    <>
      <div class="gallery-group">
        <div
          ...
          <div className="gallery-slider--wrapper">
            {props.images.map((img, ind) => (
              <div
                style={{ backgroundImage: `url('${img}')`, width: `${sliderWidth}px` }}
                key={ind}></div>
            ))}
          </div>
        </div>
        <button
          ...
          ref={buttonNext}
          onClick={() => updateSliderImage(index + 1)}></button>
        <button
          className={`gallery-slider--button prev ${index === 0 ? "disabled" : ""}`}
          ref={buttonPrev}
          onClick={() => updateSliderImage(index - 1)}></button>
        ...
        <div
          ...
          {props.images.map((i, ind) => (
            <button
              ...
              onClick={() => updateSliderImage(ind)}></button>
          ))}
        </div>
      </div>
    </>
